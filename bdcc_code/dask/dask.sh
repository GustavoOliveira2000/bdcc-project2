...
if [[ -f "/opt/conda/bin/conda" ]]; then
  readonly CONDA_BIN="/opt/conda/bin/conda"
else
  readonly CONDA_BIN="/opt/conda/miniconda3/bin/conda"
fi

if [[ -f "/opt/conda/bin/mamba" ]]; then
  readonly MAMBA_BIN="/opt/conda/bin/mamba"
else
  readonly MAMBA_BIN="/opt/conda/miniconda3/bin/mamba"
fi
...
function exit_handler() (
  ...
  # Free conda cache
  "${CONDA_BIN}" clean -a > /dev/null 2>&1

  # Clear pip cache
  pip cache purge || echo "unable to purge pip cache"

  # remove the tmpfs conda pkgs_dirs
  if [[ -d /mnt/shm ]] ; then "${CONDA_BIN}" config --remove pkgs_dirs /mnt/shm ; fi
...
function prepare_to_install() {
  ...
  local dask_env="/opt/conda/miniconda3/envs/dask"
  if [[ -d "/opt/conda/envs/p314-bdcc" ]]; then
    dask_env="/opt/conda/envs/p314-bdcc"
  elif [[ -d "/opt/conda/envs" ]]; then
    local first_env=$(ls -d /opt/conda/envs/* 2>/dev/null | head -n 1)
    if [[ -n "${first_env}" ]]; then
      dask_env="${first_env}"
    fi
  fi
  readonly DASK_CONDA_ENV="${dask_env}"
  ...
  if [[ ${free_mem} -ge 5250000 ]]; then
    ...
    "${CONDA_BIN}" config --add pkgs_dirs /mnt/shm
    mount -t tmpfs tmpfs /mnt/shm
